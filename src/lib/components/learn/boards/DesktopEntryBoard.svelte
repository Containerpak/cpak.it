<script lang="ts">
  ...
  import {
    ...
    probe,
    type Example,
  } from "$lib/learn/play/desktop-entry/examples";
  ...
  let summary = $derived.by(() => {
    if (phase === "loading") return "Loading cpak. Nothing is exported yet.";
    if (phase === "failed")
      return "cpak's own code could not be loaded, so nothing is exported.";
...
<div class="@container">
  <div class="grid gap-6 @3xl:grid-cols-12 @3xl:items-start">
    ...
    >
      ...
      <p id="entry-note" class="mt-2 text-xs leading-5 text-gray-500">
        Paste a real one. It is read in this tab and nowhere else.
      </p>
    ...
    <section
      ...
      class="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6 @3xl:col-span-7 @3xl:col-start-6 @3xl:row-span-2 @3xl:row-start-1"
    >
      ...
      {#if phase === "loading"}
        <div class="mt-6 rounded-xl bg-slate-50 px-4 py-3">
          <p class="flex items-center gap-2 leading-7 text-gray-600">
            ...
            >
            cpak is still loading. You can write the entry in the meantime.
          </p>
        </div>
      {:else if phase === "failed"}
        <div class="mt-6 rounded-xl border border-red-200 bg-white px-4 py-3">
          ...
          <p class="mt-2 leading-7 text-gray-600">
            Nothing here is guessed, so no export is shown.
          </p>
          <button
            type="button"
            onclick={load}
      ...
      {:else if missing.length > 0}
        ...
        >
          Fill in {missing.join(" and ")} first. cpak exports an entry for an
          application it installed, and the command it writes is built out of
          what the installation knows.
        </p>
      {:else if entry.trim() === ""}
        <p
          class="mt-6 rounded-xl bg-slate-50 px-4 py-3 leading-7 text-gray-600"
  ...
  {#if result}
    ...
    >
      ...
      <div class="mt-5 grid gap-6 @3xl:grid-cols-2">
        <div class="min-w-0">
  ...
  >
    ...
    <p class="mt-3 max-w-3xl leading-7 text-gray-600">
      ...
      untouched one for the launcher to run. The keys marked above are found by
      exporting the file twice with different values and watching which lines
      move, so a marked key is one cpak read rather than one this page guessed.
    </p>
  </section>
</div>
