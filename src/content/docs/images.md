...
The `main` tag follows the current platform build. `ubuntu-26.04` follows the current build for that Ubuntu release. Tags such as `ubuntu-26.04.20260814.2` and `sha-<revision>` identify one published platform state and are suitable for reproducible builds.
