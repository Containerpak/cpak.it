...
const RELEASE_API = "https://api.github.com/repos/Containerpak/cpak/releases";
...
export const GET: RequestHandler = async ({ fetch, params, url }) => {
  ...
  if (env.CPAK_INSTALLER_BASE_URL) {
    [catalogResponse, installerResponse] = await Promise.all([
      fetch(`${env.CPAK_INSTALLER_BASE_URL}/cpak-installer-catalog.json`),
      fetch(`${env.CPAK_INSTALLER_BASE_URL}/cpak-installer-linux-${arch}`),
    ]);
  } else {
    ...
    if (!releaseResponse.ok) {
      return new Response("Installer release is not available", {
        status: 503,
      });
    }
    ...
    if (
      ...
      return new Response(
        "Installer release does not match the selected release",
        {
          status: 502,
        },
      );
    ...
    const installerAsset = releaseDetails.assets.find(
      (asset) => asset.name === `cpak-installer-linux-${arch}`,
    );
    if (
      !catalogAsset ||
      !installerAsset ||
      installerAsset.size > MAX_INSTALLER_SIZE
    ) {
      return new Response("Installer release is not available", {
        status: 503,
      });
    ...
    [catalogResponse, installerResponse] = await Promise.all([
      fetch(catalogAsset.url, {
        headers: githubHeaders("application/octet-stream"),
      }),
      fetch(installerAsset.url, {
        headers: githubHeaders("application/octet-stream"),
      }),
    ]);
  }
  ...
  const installerDigest = toHex(
    await crypto.subtle.digest("SHA-256", installer),
  );
  if (details.installer_sha256 !== installerDigest) {
    return new Response(
      "Installer release does not match its signed metadata",
      {
        status: 502,
      },
    );
