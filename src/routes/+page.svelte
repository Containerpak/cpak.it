...
<section class="border-y border-slate-200 bg-white">
	<div class="mx-auto max-w-7xl px-6 py-24">
		...
		<div class="mt-14 grid divide-y divide-slate-200 border-y border-slate-200 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-3">
			{#each featuredApps as app}
				<a href={app.href} class="group flex gap-4 px-0 py-7 sm:px-7 sm:first:pl-0 lg:odd:px-7 lg:first:pl-0">
					<img src={app.icon} alt="" class="h-12 w-12 shrink-0 rounded-xl object-contain" />
					<span>
						<span class="flex items-center gap-1 font-semibold text-gray-900 group-hover:text-[#3E7BFF]">
							{app.name}
							<span class="material-symbols-outlined text-base">arrow_outward</span>
						</span>
						<span class="mt-1 block text-sm leading-6 text-gray-600">{app.description}</span>
					</span>
				</a>
			{/each}
		</div>
