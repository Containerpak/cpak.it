<script lang="ts">
  ...
  let verdict = $derived.by(() => {
    if (coreState === "loading")
      return "Loading the module. Nothing is migrated yet.";
    if (coreState === "failed")
...
<Board
  ...
  sentence="A version 1 manifest reached the host filesystem through flags, and reached host programs by naming the commands it was allowed to run. Version 2 has neither. Put the old manifest in and read the new one cpak writes for it, field by field."
...
>
  ...
  <div class="mt-6 grid items-start gap-6 lg:grid-cols-12">
    <section aria-labelledby="input" class="min-w-0 lg:col-span-5">
      ...
      {:else if coreState === "failed"}
        <div class="rounded-2xl border border-red-200 bg-red-100 p-5">
          <p class="text-sm font-semibold text-red-700">
            Nothing can be migrated without the module
          </p>
          <p class="mt-1 text-sm leading-6 text-gray-600">
            The board refuses to rewrite a manifest from memory. Try again from
