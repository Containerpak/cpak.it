  ...
  <article class="mx-auto max-w-3xl px-6 py-20">
    ...
    <div class="mt-16 space-y-7 text-lg leading-8 text-gray-700">
      ...
      <p>
        ...
        pointers, but they can refer to new bytes after every build. A manifest
        that names one of them can stay unchanged while the application behind
        it changes. That weakens review and signing while making rollback harder,
        because the name no longer identifies one result.
      </p>
      <p>
        ...
        registry.
        Publishers can continue to build with familiar tags; their release
        workflow records the resulting digest in <code>cpak.json</code> before the
        package state is signed.
      </p>
      ...
      <p>
        ...
        host display. v3 therefore replaced those broad switches with permissions
        for concrete desktop operations. Notifications, external links, file
        selection and host application launches each have their own path. Other
        session bus use can be limited to named services, objects and methods.
      </p>
      <p>
        That direction was right, but the first v3 publication stopped too early.
        Removing raw X11 did not remove the applications that still depend on
        X11. Bluetooth could not be reduced to one authentication flow either.
        It also covers discovery, pairing, GATT devices, application agents and
        profiles that carry their own connections. A permission model has to
      ...
      <p>
        cpak 2.9.5 adds <code>displayX11</code> for applications that need X11
        compatibility. cpak starts a nested Xwayland display on Wayland or a
        nested Xephyr display on X11. The package receives that display's socket
        and a private authentication cookie. It never receives the host X11
        socket or the authority file used by other applications.
      </p>
      <p>
        The new <code>bluetooth</code> permission provides the general BlueZ API
        through a private proxy at the conventional system bus path. Existing
        BlueZ clients can use discovery, pairing, GATT, agents, profiles, signals,
        callbacks and file descriptors without an application-specific protocol.
        Calls to unrelated system services are refused, bus enumeration is hidden
        and raw HCI access remains outside this permission.
      </p>
      <p>
        Neither path makes D-Bus a requirement for cpak itself. The proxy is part
        of the cpak binary and is started only for a package that declares
        Bluetooth. Hosts need BlueZ and its existing system bus for that package;
        headless packages and desktop applications that do not request Bluetooth
      ...
      <p>
        cpak 2.9.5 supersedes 2.9.4 as the recommended release and contains every
        change shipped throughout the 2.9 line. That includes manifest v3, the
        Discover backend, signed installer catalog work, the trust and sandbox
        fixes released during the week, and the new X11 and Bluetooth paths.
        Version 2.9.4 remains available for rollback, but new installations and
        updates should use 2.9.5.
      </p>
      <p>
